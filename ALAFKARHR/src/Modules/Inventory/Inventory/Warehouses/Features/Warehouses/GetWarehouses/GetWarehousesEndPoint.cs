namespace Inventory.Warehouses.Features.Warehouses.GetWarehouses
{
    ...
    public class GetWarehousesEndPoint : ICarterModule
    {
        ...
        {
            app.MapGet("/api/v1/inventory/warehouses/company/{companyId}", async ([FromRoute]Guid companyId, WarehouseType? warehouseType, Guid? branchId, [AsParameters] PaginationRequest paginationRequest, ISender sender) =>
            {
                var result = await sender.Send(new GetWarehousesQuery(companyId, paginationRequest, warehouseType, branchId));
