    ...
    : ICommandHandler<StockReleaseCommand, StockReleaseResult>
{
    public async Task<StockReleaseResult> Handle(StockReleaseCommand command, CancellationToken cancellationToken)
    {
        ...
        var tracking = await global::Inventory.Warehouses.Features.Inventories.InventoryTrackingModeGuard
            .ResolveAndValidateAsync(dbContext, sender, command.InventoryAggregate, InventorySerialOperation.Release, userId, cancellationToken);
        ...
        await global::Inventory.Warehouses.Features.Inventories.InventoryTrackingModeGuard.ApplySerialMovementAsync(
            ...
            command.InventoryAggregate.CompanyId,
            command.InventoryAggregate.SourceLocationId,
            InventorySerialOperation.Release,
            tracking.Serials,
            userId,
            cancellationToken);
