...
public class AttendanceSessionConfiguration : IEntityTypeConfiguration<AttendanceSession>
{
    public void Configure(EntityTypeBuilder<AttendanceSession> builder)
    {
        ...
        builder.Property(x => x.NormalizationStatus).HasConversion<int>().HasDefaultValue(AttendanceNormalizationStatus.Normal).IsRequired();
        ...
        builder.Property(x => x.NormalizationNote).HasMaxLength(1000);
        builder.Property(x => x.NormalizedBy).HasMaxLength(450);

        builder.HasIndex(x => x.EmployeeId);
        builder.HasIndex(x => x.ShiftId);
        builder.HasIndex(x => new { x.EmployeeId, x.Status });
        builder.HasIndex(x => new { x.CompanyId, x.NormalizationStatus });
