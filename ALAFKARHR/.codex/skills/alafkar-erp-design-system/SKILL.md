...
- When `AppCard` or another clipped container contains `AutoCompleteComponent`, Bootstrap dropdowns, popovers, menus, or similar overlays, make the clipping ancestor allow overlay overflow. For `AppCard`, preserve existing classes and add `allow-overflow`, for example `CssClass="mb-3 allow-overflow"`. Do not rely on `z-index` alone when an ancestor has `overflow: hidden`, `overflow: auto`, or table scrolling.
